...
--ctld.f10MenuName = "CTLD" -- Name of the F10 Menu entry
ctld.f10MenuName = "Transport (CTLD)" -- Name of the F10 Menu entry
ctld.spawnedCrateGroups = {}	-- stores the mist.dynAdd returned group table. Usefull for saving the unit.
ctld.aaSystemLimit = {
	[1] = 3,
	[2] = 3,
}
ctld.aaSystemsSpawned = {
	[1] = 0,
	[2] = 0,
}
ctld.numEWRs = {
...
-- *********************  USER CONFIGURATION ******************************
-- ************************************************************************
ctld.unpackDistance = 5000 			-- minimum distance from pickup zone before unpack - now 6Km
ctld.pickupZoneSpawnRadius = 1000	-- minimum distance where you are able to spawn crates - now 1000
--ctld.unpackDistance = 1 -- minimum distance from pickup zone before unpack - debugmode on = 1m
--ctld.pickupZoneSpawnRadius = 200000	-- minimum distance where you are able to spawn crates - debug mode on = 200km (or zone size, I think)
...
ctld.minimumHoverHeight = 2 -- Lowest allowable height for crate hover	-- lower really is no problem, the rope is not a stick
ctld.maximumHoverHeight = 12.0 -- Highest allowable height for crate hover
ctld.maxDistanceFromCrate = 6 -- Maximum distance from from crate for hover
...
ctld.pickupZones = {
	-- CTLD Standard
	{ "pickzone1", "blue", 200, "yes", 0 },
    { "Red_Carrier", "red", -1, "yes", 1 }, -- instead of a Zone Name you can also use the UNIT NAME of a ship
}
...
ctld.loadableGroups = {
    ...
    --{name = "Anti Air", inf = 4, aa = 1  },
    {name = "Attack Group", inf = 2, at = 3, mortar = 3, aa = 1  },
    {name = "Mortar Squad", inf = 2, mortar = 6 },
...
ctld.spawnableCrates = {
    ...
    ["FARP Groundcrew"] = {
        ...
        -- blue
        { weight = 315, desc = "Groundcrew ATC", unit = "M1025 HMMWV", side = 2, cratesRequired = 1 },	-- ATC
        { weight = 310, desc = "Groundcrew Fuel", unit = "M978 HEMTT Tanker", side = 2, cratesRequired = 1 },	-- Fuel
        { weight = 305, desc = "Groundcrew Ammo", unit = "M 818", side = 2, cratesRequired = 1 },	-- Ammo
        
        -- red
        --{ weight = 320, desc = "Groundcrew Ammo", unit = "Ural-4320T", side = 1, cratesRequired = 1 },	-- Ammo
        { weight = 320, desc = "Groundcrew Ammo", unit = "Ural-375", side = 1, cratesRequired = 1 },	-- Ammo
        { weight = 325, desc = "Groundcrew Fuel", unit = "ATZ-10", side = 1, cratesRequired = 1 },	-- Fuel
        { weight = 300, desc = "Groundcrew ATC", unit = "SKP-11", side = 1, cratesRequired = 1 },	-- ATC
        --{ weight = 330, desc = "Groundcrew PowerSupply - Zil APA-80 ", unit = "ZiL-131 APA-80", side = 1, cratesRequired = 1 },	-- electric Power
	...
	["JTAC/Artillery"] = {
        { weight = 520, desc = "JTAC HMMWV", unit = "Hummer", side = 2, }, -- used as jtac and unarmed, not on the crate list if JTAC is disabled
        { weight = 525, desc = "JTAC SKP-11", unit = "SKP-11", side = 1, }, -- used as jtac and unarmed, not on the crate list if JTAC is disabled

        --{ weight = 100, desc = "Mortar 2B11", unit = "2B11 mortar" },

		--{ weight = 500, desc = "HMMWV TOW", unit = "M1045 HMMWV TOW", side = 2 },
        
        { weight = 250, desc = "Howitzer Msta", unit = "SAU Msta", side = 1, cratesRequired = 2 },
        { weight = 255, desc = "Howitzer M-109", unit = "M-109", side = 2, cratesRequired = 2 },
        ...
        --{ weight = 900, desc = "Spawn Crate", unit = "SPAWN-CRATE", cratesRequired = 2 }, 
    },
    ["Objective Air Defenses"] = {
		--{ weight = 570, desc = "Repair", unit = "Repair"},
        ...
        { weight = 545, desc = "HAWK SR", unit = "Hawk sr", side = 2 },
        { weight = 550, desc = "HAWK TR", unit = "Hawk tr", side = 2 },
	...
	["Mobile Air Defense"] = {
		...
		-- IR Vehicles
        { weight = 400, desc = "Strela-10", unit = "Strela-10M3", cratesRequired = 1 },
        { weight = 410, desc = "Avenger", unit = "M1097 Avenger", side = 2, cratesRequired = 1 },
        
        -- Radar Guided
        { weight = 450, desc = "SA-8 Osa", unit = "Osa 9A33 ln"},
        { weight = 490, desc = "SA-15 Tor", unit = "Tor 9A331"},
        
        
        -- EWR
        { weight = 595, desc = "55G6 EWR", unit = "55G6 EWR" },
    },
}

ctld.groundCrewUnitTypes = {
    ["M1025 HMMWV"] = "ATC",
    ["M 818"] = "Ammo",
    ["M978 HEMTT Tanker"] = "Fuel",
    ["SKP-11"] = "ATC",
    ["Ural-375"] = "Ammo",
    ["ATZ-10"] = "Fuel",
}
...
function ctld.spawnCrateAtZone(_side, _weight,_zone)
    ...
    local _spawnedCrate = ctld.spawnCrateStatic(_country, _unitId, _point, _name, _crateType.weight,_side,_zone)
...
function ctld.spawnCrateStatic(_country, _unitId, _point, _name, _weight,_side,_zone)
	koEngine.debugText("ctld.spawnCrateStatic()")
    local _groupId = mist.getNextGroupId()
    --local _groupName = "Crate Group #".._groupId
    local _groupName = string.sub(_name, 1, string.find(_name, "#")-2).." Crate #".._groupId
    ...
    _group.units[1] = ctld.createUnit(_point.x , _point.z , 0, {type="UAZ-469",name=_name})
    ...
    local _spawnedCrate = Unit.getByName(_name)
    --local _spawnedCrate = coalition.addStaticObject(_country, _crate)

    local _crate = ctld.crateLookupTable[tostring(_weight)]

    --copy crate
    _crate =  { weight = _crate.weight, desc = _crate.desc, unit = _crate.unit, side = _crate.side, cratesRequired = _crate.cratesRequired, zone = _zone }

    local _copy = mist.utils.deepCopy(_crate)

    if _side == 1 then
        ctld.spawnedCratesRED[_name] =_copy
    else
        ctld.spawnedCratesBLUE[_name] = _copy
    end
...
function ctld.spawnCrate(_arguments)
	koEngine.debugText("ctld.spawnCrate()")
    local _status, _err = pcall(function(_args)
        ...
        if _crateType ~= nil and _heli ~= nil and ctld.inAir(_heli) == false then
            ...
            local _zone = ctld.inLogisticsZone(_heli)

            if _zone.inZone == false then
            ...
            local _spawnedCrate = ctld.spawnCrateStatic(_heli:getCountry(), _unitId, _point, _name, _crateType.weight,_side, _zone.name)
            
            -- TODO remember the origin zone the crate was spawned in!
            --[[if _side == 1 then
		        ctld.spawnedCratesRED[_name].zone = _zone.name
		    else
		        ctld.spawnedCratesBLUE[_name].zone = _zone.name
		    end--]]
		    ...
		    ctld.displayMessageToGroup(_heli, string.format("A %s crate has been brought out and is at your 12 o'clock ", _crateType.desc, _crateType.weight), 20)
    ...
    if (not _status) then
        koEngine.debugText(string.format("CTLD: Error in ctld.spawnCrate() %s", _err))
    end
end
...
function ctld.loadTroopsFromZone(_args)
    ...
    elseif _zone.inZone == true then
        if _zone.limit - 1 >= 0 or _zone.limit == -1 then
...
function ctld.checkHoverStatus()
                    ...
                    if _crate.dist < ctld.maxDistanceFromCrate and _crate.details.unit ~= "FOB" then
                        ...
                        if _height > ctld.minimumHoverHeight and _height <= ctld.maximumHoverHeight then
                            ...
                            if _time > 0 then
                                ...
                                ctld.displayMessageToGroup(_transUnit, "Loaded  " .. _crate.details.desc .. " crate! Bring it at least "..(ctld.unpackDistance/1000).."km or "..math.ceil((ctld.unpackDistance/1000)*0.539957).."nm away in order to unpack it!", 10,true)
                                ...
                                --koEngine.debugText("_crate after pickup = "..koEngine.TableSerialization(_crate))
                                koEngine.removeCrate(_crate.crateUnit:getName())
                                ...
                                koEngine.debugText((_transUnit:getPlayerName() or "AI").." has picked up ".._crate.details.desc)
                                
                                -- TODO hover-pickup score!
                                ctld.processCallback({unit = _transUnit, crate = _copiedCrate , action = "hoverpickup"})
                            end
    ...
    if (not _status) then
        koEngine.debugText(string.format("CTLD: Error in ctld.checkHoverStatus() %s", _result))
    end
end
...
function ctld.listNearbyCrates(_args)
    ...
    for _, _crate in pairs(_crates) do
        ...
        if _crate.dist < 1000 and _crate.details.unit ~= "FOB" and (_crate.details.zone == nil or _crate.details.zone == "")  then
        ...
        elseif _crate.dist < 1000 and _crate.details.unit ~= "FOB"  then
            _message = string.format("%s\n%s crate - kg %i - %i m - %d o'clock - from %s", _message, _crate.details.desc, _crate.details.weight, _crate.dist, ctld.getClockDirection(_heli, _crate.crateUnit),_crate.details.zone)
        end
    end
...
function ctld.displayMessageToGroup(_unit, _text, _time,_clear)
	koEngine.debugText("ctld.displayMessageToGroup '".._unit:getPlayerName().."': ".._text)
...
function ctld.getCratesAndDistance(_heli)
    ...
    for _crateName, _details in pairs(_allCrates) do
        ...
        --get crate
        local _crate = Unit.getByName(_crateName)
...
function ctld.findNearestAASystem(_heli,_aaSystem)
	koEngine.debugText("ctld.findNearestAASystem()")
    local _closestSystemGroup = nil
    local _shortestDistance = math.huge
    local _distance = 0

    for _groupName, _systemDetails in pairs(ctld.completeAASystems) do

        local _systemGroup = Group.getByName(_groupName)

        --koEngine.debugText("ctld.findNearestAASystem: ".._groupName..": ".._systemDetails[1].unit)
        local systemDetail = "nil"
        if _systemDetails[1].system then
        	systemDetail = _systemDetails[1].system.name
        end
        --koEngine.debugText("groupName: ".._groupName..", group: "..tostring(_systemGroup)..", systemDetails.name: "..systemDetail..", aaSystemName: "..tostring(_aaSystem.name))
        
        if _systemDetails then
        	--koEngine.debugText("systemDetails: "..koEngine.TableSerialization(_systemDetails))
        else
        	--koEngine.debugText("no systemDetails")
        end
        if _systemGroup and _systemGroup:getCoalition() == _heli:getCoalition() and _systemDetails[1].system and _systemDetails[1].system.name == _aaSystem.name then
        	--koEngine.debugText("criteria matched")

            local _units = _systemGroup:getUnits()
            ...
            for _, _leader in pairs(_units) do
                ...
                if _leader ~= nil and _leader:getLife() > 0 then
                    ...
                    if _distance ~= nil and _distance < _shortestDistance then
                        _shortestDistance = _distance
                        _closestSystemGroup = _systemGroup
                    end
    ...
    if _closestSystemGroup ~= nil then

        return { group = _closestSystemGroup, dist = _shortestDistance }
    end
    return nil
end

function ctld.destroyOldestSam(coa)
	koEngine.debugText("ctld.destroyOldestSam('"..coa.."'")
	
	if not MissionData then koEngine.debugText("CTLD: MissionData invalid! ") return end
			
	local oldestSam = false
	local oldestAge = 0
	local oldestIndex = -1
	...
	for i, sam in pairs(MissionData.properties.droppedSAMs) do
		...
		-- check age of sam: 
		if sam.coalition == coa and not sam.groupName:find("Igla") and not sam.groupName:find("Stinger") and not sam.groupName:find("JTAC") and not sam.groupName:find("EWR") then 
	...
	if oldestSam then
		...
		local units = oldestSam.spawnedGroup:getUnits()
		for j, unit in pairs(units) do
			local unitPoint = unit:getPoint()
			point.x = point.x + unitPoint.x
			point.y = point.y + unitPoint.y
			point.z = point.z + unitPoint.z
		end
		
		point.x = point.x / #units
		point.y = point.y / #units
		point.z = point.z / #units
		
		koEngine.outTextForCoalition(coa, "AA System Limit has been reached! "..oldestSam.groupName.." been destroyed!", 20)
		mist.scheduleFunction(trigger.action.explosion, {point, 5000 }, timer.getTime()+0.1)
		oldestSam.spawnedGroup:destroy()
		
		--remove from Mission Data and CTLD
		ctld.completeAASystems[MissionData.properties.droppedSAMs[oldestIndex].groupName] = nil
		table.remove(MissionData.properties.droppedSAMs, oldestIndex)
	else
		koEngine.debugText("oldest Sam not found")
	end
end
...
function ctld.unpackCrates(_arguments)
    ...
    local _status, _err = pcall(function(_args)
        ...
        if _heli ~= nil and ctld.inAir(_heli) == false then
            ...
            elseif _crate ~= nil and _crate.dist < 200 then
				...
				--koEngine entrypoint, check for ground-unit-crates here!
                local _zone = ctld.inPickupZone(_heli)
               -- koEngine.debugText("CTLD.unpackCrates(): _zone = "..koEngine.TableSerialization(_zone))
                koEngine.debugText("Player '".._heli:getPlayerName().."' unpacks a '".._crate.details.desc.."'")
                
                local inMissionZone = koEngine.isUnitInObjectiveZone(_heli)

				--koEngine.debugText("crate: "..koEngine.TableSerialization(_crate))
				
				-- TODO: can only not deploy in the zone it was packed!
                if (inMissionZone and inMissionZone == _crate.details.zone) then
					koEngine.outTextForGroup(ctld.getGroupId(_heli), "You can't unpack a crate in the same zone where you picked it up! \n\nTake it at least "..(ctld.unpackDistance/1000).."km/"..(math.ceil(mist.utils.metersToNM(ctld.unpackDistance*10))/10).."nm away from where you picked it up!", 30)
					
                	return -- TODO uncomment this! CHEAT to unpack anywhere
                elseif (not _zone.inZone and (_zone.dist < ctld.unpackDistance and _zone.dist ~= -1)) and not (inMissionZone and inMissionZone ~= _crate.details.zone) then
                	koEngine.outTextForGroup(ctld.getGroupId(_heli), "You can't unpack that here! \n\nTake it at least "..(ctld.unpackDistance/1000).."km/"..(math.ceil(mist.utils.metersToNM(ctld.unpackDistance*10))/10).."nm away from where you picked it up!\nYou are currently "..(math.ceil(_zone.dist/100)/10).."km away from your pickup zone", 30)
                ...
                if _aaTemplate then
                    ...
                    if _crate.details.unit == _aaTemplate.repair then
                        ctld.repairAASystem(_heli, _crate,_aaTemplate, inMissionZone)
                    else
                    	-- check AA System limit!
                    	local coa = _heli:getCoalition()
                    	local dropzone = koEngine.isUnitInObjectiveZone(_heli)
                    	koEngine.debugText("CTLD: spawning crate, dropzone = "..tostring(dropzone))
                    	
                    	-- if we are in a dropzone, deploy the system regardless limit!
                    	if dropzone then
                    		--koEngine.debugText("CTLD: spawning crate in dropzone")
                    		ctld.unpackAASystem(_heli, _crate, _crates,_aaTemplate, dropzone)
                    		
                    	elseif not dropzone and ctld.aaSystemLimit[coa] > 0  then
                    		--koEngine.debugText("CTLD: not in a dropzone but systemlimit > 0")
	                    	
	                        ctld.unpackAASystem(_heli, _crate, _crates,_aaTemplate, dropzone)
	                        
	                        if ctld.aaSystemsSpawned[coa]+1 > ctld.aaSystemLimit[coa] then
	                    		-- find the oldest sam-system and delete it before spawn
	                    		--ctld.displayMessageToGroup(_heli, "You have already placed "..ctld.aaSystemsSpawned[coa].." AA Systems! You cant unpack more", 20)
	                    		koEngine.debugText("SAM-Limit reached, destroying oldest sam")
	                    		ctld.destroyOldestSam(coa)
	                    	else
	                    		ctld.aaSystemsSpawned[coa] = ctld.aaSystemsSpawned[coa] + 1 
	                        end
                        else
                        	-- message: no SAM Systems available
                        	koEngine.outTextForGroup(ctld.getGroupId(_heli), "There are no SAM-Systems available. Occupy an Antenna to receive a SAM-System to be deployed in the wild or bring the crate to an Objective. \n\nCheck the live-map to find their locations or use the Mission-Status F10 Menu!", 30)
                        end
                    end
                ...
                elseif _crate.details.cratesRequired ~= nil and _crate.details.cratesRequired > 1 then
                	...
                	-- TODO: implement restrictions for Kaukasus Offensive
                    -- single crate
                    ...
                    if not dropzone and ctld.aaSystemLimit[coa] == 0 and 
                    	not string.find(_crate.details.desc, "Igla") and not string.find(_crate.details.desc, "Stinger") and 
                    	not string.find(_crate.details.desc, "EWR") and not string.find(_crate.details.desc, "JTAC") then
                    ...
                    local _spawnedGroups = ctld.spawnCrateGroup(_heli, { _cratePoint }, { _crate.details.unit }, _crateName)
                    ...
                    if _crate.details.unit == "1L13 EWR" or _crate.details.unit == "55G6 EWR" then
					...
					if dropzone then
                    	koEngine.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. " successfully deployed " .. _crate.details.desc .. " to "..dropzone, 10)
                    else
                    	koEngine.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. " successfully deployed " .. _crate.details.desc .. " in the wild", 10)
                    end

                    if ctld.isJTACUnitType(_crate.details.unit) and ctld.JTAC_dropEnabled then
						
                        local _code = table.remove(ctld.jtacGeneratedLaserCodes, 1)
                        _code = 1688 -- enable mirage standard code
                        --put to the end
                    ...
                    if not dropzone and not string.find(_crate.details.desc, "Igla") and not string.find(_crate.details.desc, "Stinger") and 
                    	not string.find(_crate.details.desc, "EWR") and not string.find(_crate.details.desc, "JTAC")then
                    	--koEngine.debugText("CTLD: not in a dropzone but systemlimit > 0")
                    	if ctld.aaSystemsSpawned[coa]+1 <= ctld.aaSystemLimit[coa] then
                    		ctld.aaSystemsSpawned[coa] = ctld.aaSystemsSpawned[coa] + 1
    ...
    if (not _status) then
        koEngine.debugText(string.format("CTLD: Error in ctld.unpackCrates(): %s", _err))
    end
end
...
function ctld.dropSlingCrate(_args)
	koEngine.debugText("ctld.dropSlingCrate()")
    local _heli = ctld.getTransportUnit(_args[1])
    ...
    if _currentCrate == nil then
        ctld.displayMessageToGroup(_heli, "You are not currently transporting any crates. \n\nTo Pickup a crate, hover for 4 seconds above the crate", 10)
    else
    	
        local _heli = ctld.getTransportUnit(_args[1])
        local playerName = _heli:getPlayerName() or "AI"

		local zone = koEngine.isUnitInObjectiveZone(_heli)
        local _point = _heli:getPoint()
        --koEngine.debugText("_point.y = ".._point.y)
        --koEngine.debugText("getHeight() = "..land.getHeight({ x = _point.x, y = _point.z }))
        _point.y = land.getHeight({ x = _point.x, y = _point.z }) + 1	-- fix heli crashing into the crate thus destroying it
        --koEngine.debugText("_point.y ) ".._point.y)
        ...
        if ctld.inAir(_heli) == false or _heightDiff <= 7.5 then
            ...
            ctld.processCallback({unit = _heli, crate = _currentCrate.desc , action = "crate_safely_unhooked", zone = zone})
        ...
        elseif _heightDiff > 7.5 and _heightDiff <= 40.0 then
        	_point = ctld.getPointAt12Oclock(_heli, 20)
            ctld.displayMessageToGroup(_heli, _currentCrate.desc .. " crate has been safely dropped below you", 10)
            ctld.processCallback({unit = _heli, crate = _currentCrate.desc , action = "crate_safely_dropped", zone = zone})
        else -- _heightDiff > 40.0
	        ctld.inTransitSlingLoadCrates[_heli:getName()] = nil
	        ctld.displayMessageToGroup(_heli, "You were too high! The crate has been destroyed", 10)
	        ctld.processCallback({unit = _heli, crate = _currentCrate.desc , action = "crate_dropped_on_ground", zone = zone})
        return
        end
        ...
        -- TODO
        -- save the crate in MissionData
        local newCrate = {
        	country = _heli:getCountry(),
        	point = _point,
        	name = _name,
        	weight = _currentCrate.weight,
        	side = _side,
        	zone = _currentCrate.zone,
        	spawnTime = MissionData.properties.campaignSecs,
        	playerName = playerName,
        }
        
        
        MissionData.properties.droppedCrates = MissionData.properties.droppedCrates or {}
        table.insert(MissionData.properties.droppedCrates, newCrate)

        local _spawnedCrate = ctld.spawnCrateStatic(_heli:getCountry(), _unitId, _point, _name, _currentCrate.weight,_side, _currentCrate.zone)
    end
end
...
function ctld.rearmAASystem(_heli, _nearestCrate, _nearbyCrates, _aaSystemTemplate)
	koEngine.debugText("ctld.rearmAASystem()")
    ...
    if ctld.getLauncherUnitFromAATemplate(_aaSystemTemplate) == _nearestCrate.details.unit then
        ...
        if _nearestSystem ~= nil and _nearestSystem.dist < 300 then
            ...
            if ctld.countTableEntries(_uniqueTypes) == _aaSystemTemplate.count and #_points >= _aaSystemTemplate.count then
                ...
                local _oldGroup = _nearestSystem.group

                _nearestSystem.group:destroy()

                local _spawnedGroup = ctld.spawnCrateGroup(_heli, _points, _types, _aaSystemTemplate.name)

                ctld.completeAASystems[_spawnedGroup:getName()] = ctld.getAASystemDetails(_spawnedGroup, _aaSystemTemplate)

                ctld.processCallback({unit = _heli, crate =  _nearestCrate , spawnedGroup = _spawnedGroup, replacedGroup = _oldGroup, action = "rearm"})
                ...
                koEngine.removeCrate(_nearestCrate.crateUnit:getName())
...
function ctld.getAASystemDetails(_systemGroup,_aaSystemTemplate)
	if not _aaSystemTemplate then
		--koEngine.debugText("ctld.getAASystemDetails() - _aaSystemTempalte = nil, returning nil")
		return nil
	end
...
function ctld.unpackAASystem(_heli, _nearestCrate, _nearbyCrates,_aaSystemTemplate, dropzone)
	koEngine.debugText("ctld.unpackAASystem")
    ...
    for _name, _systemPart in pairs(_systemParts) do
        ...
        if _systemPart.found == false then
            ...
            if (_name == "Hawk ln" and ctld.hawkLaunchers > 1) or ((_name == "Kub 2P25 ln" or _name == "SA-11 Buk LN 9A310M1") and ctld.aaLaunchers > 1) 
    ...
    if _txt ~= "" then
        ...
        for _name, _systemPart in pairs(_systemParts) do
            ...
            koEngine.removeCrate(_systemPart.crate.crateUnit:getName())
        ...
        -- HAWK / BUK READY!
        local _spawnedGroup = ctld.spawnCrateGroup(_heli, _posArray, _typeArray, _aaSystemTemplate.name)
		...
		if dropzone then
        	trigger.action.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. " successfully deployed a full ".._aaSystemTemplate.name.." to "..dropzone, 10)
        else
        	trigger.action.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. " successfully deployed a full ".._aaSystemTemplate.name.." in the wild", 10)
		end
    end
end


function ctld.repairAASystem(_heli, _nearestCrate,_aaSystem, zone)
	koEngine.debugText("ctld.repairAASystem()")
    -- find nearest COMPLETE AA system
    local _nearestSystem = ctld.findNearestAASystem(_heli,_aaSystem)
		
	koEngine.debugText("nearestSystem = "..koEngine.TableSerialization(_nearestSystem))
    if _nearestSystem ~= nil and _nearestSystem.dist < 300 then
		koEngine.debugText("repairing ".._aaSystem.name)
		koEngine.debugText("_aaSystem = "..koEngine.TableSerialization(_aaSystem))
        local _oldSystem = ctld.completeAASystems[_nearestSystem.group:getName()]
		local _replacedGroup = _nearestSystem.group
		local _replacedGroupName = _nearestSystem.group:getName()
        --spawn new one

        local _types = {}
        local _points = {}
        
        local completeParts = mist.utils.deepCopy(_aaSystem.parts)	-- all parts needed to spawn the unit

        -- old CTLD code wont work with damaged but respawned units, make it obsolete
        --[[for _, _part in pairs(_oldSystem) do
            table.insert(_points, _part.point)
            table.insert(_types, _part.unit)
        end--]]
        local numLaunchers = ctld.aaLaunchers	-- make sure all launchers are there
        
        --koEngine.debugText("_oldSystem = "..koEngine.TableSerialization(_oldSystem))
        
        for _, _part in pairs(_oldSystem) do
            table.insert(_points, _part.point)
            table.insert(_types, _part.unit)
            
            --koEngine.debugText("_part.point = "..koEngine.TableSerialization(_part.point))
            
            -- remove the part from the _aaSystem template!
            for i, part in pairs(completeParts) do
            	if part.name == _part.unit then
            		if part.launcher and numLaunchers > 1 then
            			--koEngine.debugText("numLaunchers > 1 -> launcher removed")
            			numLaunchers = numLaunchers - 1
            		else
            			--koEngine.debugText("removing part '"..part.desc.."' from _oldSystem")
            			table.remove(completeParts,i)
            		end            		
            	end
            end
        end
        
        local prevPoint = _points[#_points]
        local numPartsMissing = #completeParts
        koEngine.debugText("there are "..numPartsMissing.." parts missing from spawned group compared to template!")
        for i, missingPart in pairs(completeParts) do
        	-- calculate a position for the missing point, then add data to the tables used to span the repaired sam
        	-- spawn in a circle around the last unit
            local _angle = math.pi * 2 * (i - 1) / numPartsMissing
            local _xOffset = math.cos(_angle) * 12
            local _yOffset = math.sin(_angle) * 12
            
			table.insert(_points, { x = (prevPoint.x + _xOffset), y = prevPoint.y, z = (prevPoint.z + _yOffset) })
        	table.insert(_types, missingPart.name)
        	
        	koEngine.debugText("ctld.repair: added missing '"..missingPart.name.."'")
        end
        
        -- look of _aaSystem table
        --[[{
        name = "HAWK AA System",
        count = 4,
        parts = {
        ...
        --remove old system
        ctld.completeAASystems[_replacedGroupName] = nil
        
        
        _nearestSystem.group:destroy()
        
        local _spawnedGroup = ctld.spawnCrateGroup(_heli, _points, _types, _aaSystem.name)
        
        ctld.completeAASystems[_spawnedGroup:getName()] = ctld.getAASystemDetails(_spawnedGroup,_aaSystem)
        
        ctld.processCallback({unit = _heli, crate = _nearestCrate , spawnedGroup = _spawnedGroup, replacedGroup = _replacedGroup, replacedGroupName = _replacedGroupName, action = "repair", zone = zone})

        koEngine.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. " successfully repaired a full ".._aaSystem.name.." in the field", 10)
    ...
    koEngine.debugText("ctld.repairAASystem() - finished!")
end
...
function ctld.unpackMultiCrate(_heli, _nearestCrate, _nearbyCrates)
    ...
    if #_nearbyMultiCrates == _nearestCrate.details.cratesRequired then
        ...
        local _spawnedGroup = ctld.spawnCrateGroup(_heli, { _point }, { _nearestCrate.details.unit }, _nearestCrate.details.desc)
...
function ctld.spawnCrateGroup(_heli, _positions, _types, _name)

	
    local _id = mist.getNextGroupId()

	local playerName = koEngine.getPlayerNameFix(_heli:getPlayerName()) or "AI"
	
	if not _name then
	    _name = _types[1]
	end
	
	koEngine.debugText("ctld.spawnCrateGroup("..playerName..", "..#_positions.." positions, "..#_types.." types, name = ".._name..")")
	
	if _name:find("#") then
		_name = _name:sub(1, _name:find("#")-2)
	end
	local _groupName = playerName.."s ".._name
	...
	if MissionData then 
		...
		_groupName = _groupName .. " #"..numSams
		koEngine.debugText("ctld.spawnCrateGroup(".._types[1].."): MissionData available, groupName = ".._groupName)
	else
		_groupName = playerName.."s ".._name.. " #" .. #ctld.spawnedCrateGroups
	end
    ...
    if #_positions == 1 then
        ...
        local _unitId = mist.getNextUnitId()
        --local _details = { type = _types[1], unitId = _unitId, name = string.format("%ss %s #%i", playerName, _types[1], _unitId) }
        local _details = { type = _types[1], name = string.format("%ss %s #%i", playerName, _types[1], numSams) }
        ...
        for _i, _pos in ipairs(_positions) do
            ...
            local _unitId = mist.getNextUnitId()
            --local _details = { type = _types[_i], unitId = _unitId, name = string.format("%ss %s #%i", playerName, _types[_i], _unitId) }
            local _details = { type = _types[_i], name = string.format("%ss %s #%i #%i", playerName, _types[_i], numSams, _i) }

            _group.units[_i] = ctld.createUnit(_pos.x + 5, _pos.z + 5, 120, _details)
        end
    end

	--TODO save dynAdd returned group
    --mist function
    _group.category = Group.Category.GROUND
    _group.country = _heli:getCountry()

	-- keep a deep copy of the spawn table 
	ctld.spawnedCrateGroups[_groupName] = mist.utils.deepCopy(_group)
	
	--koEngine.debugText("spawngroup = "..koEngine.TableSerialization(_group))
...
-- TODO add calsign and frequency to addEWRTask()
function ctld.addEWRTask(_group)
	env.info("ctld.addEWRTask("..tostring(_group)..")")
    -- delayed 2 second to work around bug
    timer.scheduleFunction(function(_ewrGroup)
        local _grp = ctld.getAliveGroup(_ewrGroup)
		
        if _grp then
        	local side = _grp:getCoalition()
        	ctld.numEWRs[side] = ctld.numEWRs[side] + 1
        	env.info("ctld.addEWRTask(): EWR ".._ewrGroup.." gets number "..ctld.numEWRs[side])
            local _controller = _grp:getController()
            local _EWR = {
            	enabled = true,
                id = 'EWR',
                auto = true,
                number = ctld.numEWRs[side],
                params = {
                },
            }
            
            local callsign
            local callName = ctld.numEWRs[side]
        	local number = math.floor(callName / #ctld.ewrCallsigns) + 1
        	if callName >  #ctld.ewrCallsigns then
        		callName = callName - ((number-1) * #ctld.ewrCallsigns)
        	end
            
            env.info("side = "..side)
            if side == 1 then		-- red ewr
            	callsign = {
	            	id = 'SetCallsign',
	            	params = {
	            		callname = callName,
            ...
            env.info("ctld.addEWRTask(): setting callsign: "..ctld.ewrCallsigns[callsign.params.callname]..", number: "..callsign.params.number)
            ...
            _controller:setTask(callsign)
            _controller:setTask(frequency)
        end
    end
...
function ctld.inPickupZone(_heli)
    ...
    local _closestDist = -1
    local _closestIndex = -1
    local _closestLimit = -1
    local _zoneName = ""
    local _zoneCoalition = 0
    ...
    for _i, _zoneDetails in pairs(ctld.pickupZones) do
        ...
        if _triggerZone ~= nil then
            ...
            if _zoneDetails[4] == 1 and (_zoneDetails[5] == _heliCoalition or _zoneDetails[5] == 0) then
                ...
                if _dist < _closestDist or _closestDist == -1 then
                    _closestDist = _dist
                    _closestIndex = _i
                    _closestLimit = _zoneDetails[3]
                    _zoneName = _zoneDetails[1]
                    _zoneCoalition = _zoneDetails[5]
                end
            end
        end
    end
    ...
    for _, _fob in ipairs(_fobs) do
        ...
        if _dist <= 150 then
        	env.info("WARNING: CTLD think thers a spawned FOB!")
            return { inZone = true, limit = 10000, index = -1 };
        end
    end

	-- 
	if _closestDist <= ctld.pickupZoneSpawnRadius and _dist ~= -1 then
        return { inZone = true, limit = _closestLimit, index = _closestIndex, dist = _closestDist, name = _zoneName, coalition = _zoneCoalition }
    end

    return { inZone = false, limit = _closestLimit, index = _closestIndex, dist = _closestDist };
end
...
function ctld.inLogisticsZone(_heli)
    return ctld.inPickupZone(_heli)
end
...
function ctld.processCallback(_callbackArgs)
    ...
    for _, _callback in pairs(ctld.callbacks) do
        ...
        if (not _status) then
            koEngine.debugText(string.format("CTLD: Error in ctld.processCallback() %s", _result))
        end
    end
end
    ...
    for _, _unitName in pairs(ctld.transportPilotNames) do
        local status, error = pcall(function()
            ...
            if _unit ~= nil and _unit:getPlayerName() == nil then
                ...
                --  koEngine.debugText("Checking.. ".._unit:getName())
                if _zone.inZone == true and not ctld.troopsOnboard(_unit, true) then
                    --   koEngine.debugText("in zone, loading.. ".._unit:getName())
                ...
                elseif ctld.inDropoffZone(_unit) and ctld.troopsOnboard(_unit, true) then
                    --     koEngine.debugText("in dropoff zone, unloading.. ".._unit:getName())
                    ctld.unloadTroops( { _unitName, true })
                end
        ...
        if (not status) then
            koEngine.debugText(string.format("CTLD: Error with ai status: %s", error), false)
        end
    end
...
function ctld.addF10MenuOptions(_dontSchedule)
    -- Loop through all Heli units
	--env.info("ctld.addF10MenuOptions()")
	
	if not _dontSchedule then
            ...
            if _unit ~= nil then
                ...
                if _groupId then
                    ...
                    if ctld.addedTo[tostring(_groupId)] == nil then
						...
						local _rootPath = missionCommands.addSubMenuForGroup(_groupId, ctld.f10MenuName)
                 ...
                 --env.info(string.format("CTLD.addF10MenuOptions() - unit nil %s",_unitName))
            end
        end)

        if (not status) then
            koEngine.debugText(string.format("CTLD: Error adding f10 to transport: %s", error), false)
        end
    end
    ...
    if (not status) then
        koEngine.debugText(string.format("CTLD: Error adding f10 to other players: %s", error), false)
    end
...
function ctld.laseUnit(_enemyUnit, _jtacUnit, _jtacGroupName, _laserCode)
    ...
    if _oldLase == nil or _oldIR == nil then
        ...
        local _status, _result = pcall(function()
            ...
            --1113 Su-25T lasercode
            return _spots
        end)

        if not _status then
            koEngine.debugText('ERROR: ' .. _result, false)
        else
...
--trigger.action.outText("CTLD finished", 10)
